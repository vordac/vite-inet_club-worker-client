...
function Sidebar({ role, tab, setTab }) {
    ...
    if (role === "director") {
        ...
        return (
            <div className={`aside`}>
                <button onClick={() => handleTabClick(1)}>Повідомлення</button>
                <button onClick={() => handleTabClick(2)}>Резервації</button>
                <button onClick={() => handleTabClick(3)}>Місця</button>
                <button onClick={() => handleTabClick(4)}>Тарифи</button>
                <button onClick={() => handleTabClick(5)}>Пристрої</button>
            </div>
        )
    } else {
        return (
